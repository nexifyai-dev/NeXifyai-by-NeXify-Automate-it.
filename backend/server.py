...
async def list_projects(
    ...
    projects = []
    async for p in db.projects.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit):
        # Completeness
...
async def get_project(project_id: str, current_user: dict = Depends(get_current_admin)):
    ...
    # Load sections
    sections = []
    async for s in db.project_sections.find({"project_id": project_id}, {"_id": 0}).sort("created_at", 1):
        sections.append(s)
    # Load latest chat messages
    chat = []
    async for m in db.project_chat.find({"project_id": project_id}, {"_id": 0}).sort("timestamp", -1).limit(30):
        chat.append(m)
    chat.reverse()
    # Latest version
    latest_version = await db.project_versions.find_one(
...
async def update_project(project_id: str, data: dict, current_user: dict = Depends(get_current_admin)):
    ...
    old = await db.projects.find_one({"project_id": project_id}, {"_id": 0, "status": 1, "customer_email": 1, "contact_id": 1})
    if not old:
        raise HTTPException(404, "Projekt nicht gefunden")
...
@app.post("/api/admin/projects/{project_id}/sections")
async def upsert_project_section(project_id: str, data: dict, current_user: dict = Depends(get_current_admin)):
    ...
    if not project:
        raise HTTPException(404, "Projekt nicht gefunden")
    # Check existing
    existing = await db.project_sections.find_one(
        {"project_id": project_id, "section_key": section_key}, {"_id": 0}
    )
    if existing:
        new_version = existing.get("version", 1) + 1
        await db.project_sections.update_one(
        ...
        await db.project_sections.insert_one({**section})
...
@app.get("/api/admin/projects/{project_id}/sections")
...
@app.post("/api/admin/projects/{project_id}/sections/{section_key}/review")
...
@app.post("/api/admin/projects/{project_id}/chat")
async def add_project_chat(project_id: str, data: dict, current_user: dict = Depends(get_current_admin)):
    ...
    await db.project_chat.insert_one({**msg})
    msg.pop("_id", None)
    if hasattr(msg.get("timestamp"), "isoformat"):
        msg["timestamp"] = msg["timestamp"].isoformat()
    return msg
...
@app.get("/api/admin/projects/{project_id}/chat")
async def get_project_chat(project_id: str, limit: int = 50, current_user: dict = Depends(get_current_admin)):
    """Projektchat-Verlauf."""
    messages = []
    async for m in db.project_chat.find({"project_id": project_id}, {"_id": 0}).sort("timestamp", -1).limit(limit):
...
@app.post("/api/admin/projects/{project_id}/build-handover")
async def generate_build_handover(project_id: str, data: dict = None, current_user: dict = Depends(get_current_admin)):
    ...
    await db.project_versions.insert_one({**version_doc})
    await db.projects.update_one(
        {"project_id": project_id},
...
@app.get("/api/admin/projects/{project_id}/versions")
async def list_project_versions(project_id: str, current_user: dict = Depends(get_current_admin)):
    ...
    versions = []
    async for v in db.project_versions.find({"project_id": project_id}, {"_id": 0}).sort("version", -1):
...
async def customer_projects(current_user: dict = Depends(get_current_customer)):
    ...
    projects = []
    async for p in db.projects.find({"customer_email": email}, {"_id": 0}).sort("created_at", -1):
        sections_status = p.get("sections_status", {})
        total = len(PROJECT_SECTIONS)
        done = sum(1 for v in sections_status.values() if v in ("freigegeben", "review"))
        p["completeness"] = round(done / total * 100) if total else 0
...
async def customer_project_detail(project_id: str, current_user: dict = Depends(get_current_customer)):
    ...
    email = current_user["email"]
    project = await db.projects.find_one({"project_id": project_id, "customer_email": email}, {"_id": 0})
    if not project:
        raise HTTPException(404, "Projekt nicht gefunden")
    ...
    chat = []
    async for m in db.project_chat.find(
...
@app.post("/api/customer/projects/{project_id}/chat")
async def customer_project_chat(project_id: str, data: dict, current_user: dict = Depends(get_current_customer)):
    ...
    email = current_user["email"]
    project = await db.projects.find_one({"project_id": project_id, "customer_email": email}, {"_id": 0, "project_id": 1})
    if not project:
        raise HTTPException(404, "Projekt nicht gefunden")
    content = data.get("content", "")
    if not content.strip():
...
async def list_contracts(
    ...
    contracts = []
    async for c in db.contracts.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit):
...
@app.patch("/api/admin/contracts/{contract_id}")
async def update_contract(contract_id: str, data: dict, current_user: dict = Depends(get_current_admin)):
    ...
    if not updates:
        raise HTTPException(400, "Keine gültigen Felder")
    updates["updated_at"] = utcnow()
    old = await db.contracts.find_one({"contract_id": contract_id}, {"_id": 0, "status": 1, "customer": 1, "version": 1})
    if not old:
        raise HTTPException(404, "Vertrag nicht gefunden")
...
@app.post("/api/admin/contracts/{contract_id}/appendices")
async def add_contract_appendix(contract_id: str, data: dict, current_user: dict = Depends(get_current_admin)):
    ...
    contract = await db.contracts.find_one({"contract_id": contract_id}, {"_id": 0, "contract_id": 1})
    if not contract:
        raise HTTPException(404, "Vertrag nicht gefunden")
    ...
    await db.contract_appendices.insert_one({**appendix})
    # Track in contract
    await db.contracts.update_one(
        {"contract_id": contract_id},
...
@app.get("/api/admin/contracts/{contract_id}/evidence")
...
async def customer_contract_detail(contract_id: str, current_user: dict = Depends(get_current_customer)):
    ...
    if contract.get("status") == ContractStatus.SENT.value:
        await db.contracts.update_one({"contract_id": contract_id}, {"$set": {"status": ContractStatus.VIEWED.value, "viewed_at": utcnow().isoformat()}})
...
@app.post("/api/customer/contracts/{contract_id}/accept")
async def customer_accept_contract(contract_id: str, data: dict, request: Request, current_user: dict = Depends(get_current_customer)):
    ...
    await db.contract_evidence.insert_one({**evidence})
    # Update contract
    await db.contracts.update_one(
        {"contract_id": contract_id},
        {"$set": {
...
@app.post("/api/customer/contracts/{contract_id}/decline")
...
@app.post("/api/customer/contracts/{contract_id}/request-change")
